...
/** Title-case a webhook summary ("File created" → "File Created"). The
 *  n8n picker automatically prepends "On " to every trigger multi-option
 *  entry, so we deliberately do NOT include it here — otherwise the
 *  picker renders "On On File Created". */
	...
	const eventOptions = allEvents
		.map((e) => {
			...
			return [
				...
				`\t\t\t\t\tdescription: ${tsString(e.summary)},`,
				`\t\t\t\t},`,
			].join('\n');
		})
		.join('\n');
