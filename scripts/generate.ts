		...
		credentialTest: {
			async imagekitWebhookSecretTest(
				...
				if (!secret.startsWith('whsec_')) {
					return {
						status: 'Error',
						message: 'Webhook secret must start with "whsec_" (copy it from the ImageKit dashboard).',
					};
				}
				try {
				...
				return {
					status: 'OK',
					message: 'Webhook secret format is valid. Real signature verification happens when ImageKit delivers a webhook.',
				};
			},
		},
	};
